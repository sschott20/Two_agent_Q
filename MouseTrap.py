...
def start_game(template, delay=0, output=True):
    ...
    while not game_end:
        ...
        q, c = (
            wall_position[0] + mouse_position[0] - 2,
            wall_position[1] + mouse_position[1] - 2,
        )
...
if __name__ == "__main__":
    ...
    temp_winlog = []
    # print("", " games in ", round(time.time() - start_time, 2), "seconds")
    print("500,000 games")
    for i in range(100):
        temp_winlog.append(start_game(board_template, output=False))
    temp_mouse_win = 0
    temp_trapper_win = 0
    for i in temp_winlog:
        if i == "Win":
            temp_mouse_win += 1
        else:
            temp_trapper_win += 1
    print(f"M: {temp_mouse_win}, T: {temp_trapper_win}")
    print("\n")
    for i in range(5):
        temp_winlog.append(start_game(board_template, output=True))

    for i in range(training_steps + 1):
        if i % 10000 == 0:
            print(i, " games in ", round(time.time() - start_time, 2), "seconds")
            # temp_winlog = []
            # for i in range(100):
            #     temp_winlog.append(start_game(board_template, output=False))
            # temp_mouse_win = 0
    ...
    print(
        f" --- {training_steps} games completed over {round(time.time() - start_time, 2)} seconds --- "
    )
    mouse.output_table("final")
    trapper.output_table("final")
    for i in range(5):
